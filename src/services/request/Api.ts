...
class Api {
    ...
    buildHeaders = (options:any): object => {
        const headers = {
            Authorization: `${config.tokenType} ${this._token || Cookies.get("accessToken") || ""}`,
            ...options?.headers
        };
        return headers;
    };

    axios = (method: Method, url: string, data?: object, options?: any): AxiosPromise => {
    ...
    get = async (path: string, params?: object, options = {}): Promise<AxiosResponse> => {
        return await this.axios("GET", path, params, options);
    };
    post = async (path: string, body?: object, options = {}): Promise<AxiosResponse> => {
        return await this.axios("POST", path, body, options);
    };
    put = async (path: string, body: object, options = {}): Promise<AxiosResponse> => {
        return await this.axios("PUT", path, body, options);
    };
    delete = async (path: string, params?: object, options = {}): Promise<AxiosResponse> => {
